...
mycurser.execute("Create database if not exists Bank_db")
mycurser.execute("use Bank_db")
...
mycurser.execute("create table if not exists Transaction(Acc_no int,Amount int(12),Tx_type char(1),"
                 "foreign key(Acc_no) references Bank_Account(Acc_no) on update cascade on delete cascade)")
